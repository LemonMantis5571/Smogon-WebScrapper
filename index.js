...
app.get("/api/smogon/:pokemon", async (req, res) => {
    ...
    try {
        ...
        const setSummary = await page.evaluate(() => {
            ...
            const set = [];
            const evsList = [];
            const item = setSummary[0].querySelector('.ItemLink').querySelector('span').querySelector('span:nth-child(3)').textContent.trim();

            const ability = setSummary[0].querySelectorAll("td")[1].querySelector(".AbilityLink").querySelector("span").textContent.trim();
